...
def extract_chosen_isolates(
    chosen_isolates_csv: pd.DataFrame, matrix_EU: pd.DataFrame
) -> pd.DataFrame:
    """
    Select the chosen isolates from the script. Return a DataFrame only containing the rows of selected isolates
    """
    chosen_rows = matrix_EU["Isolate"].isin(chosen_isolates_csv["Isolate"])
...
def clean_SIR_data(SIR_data_list: list):
    cleaned_SIR_data = []
    for antibiotic, SIR_data in SIR_data_list:
        if parse_SIR(SIR_data):
            SIR_category = SIR_data[0]
