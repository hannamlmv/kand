...
def species_fill(
    ...
    """
    Inputs:
    - chosen_isolates [DataFrame]: Contains chosen isolates
    - available_data [DataFrame]: sorted_dataset (from main-function)
    - temp_data [DataFrame]: all rows in sorted_dataset where the pathogen is pat (from isolate_selection)
    - parameters: [Dictionary]: from .json file
    - isos_req: [int]: Parameter retrieved from parameters_setttings.json. Number of isolates required for a pat
    - errors [DataFrame]:
    - pat [string]: Pathogen to select isolates for
    """

    # If you want to select the lower limit of isolates. Change settings for "Lower limit" to "True"
    ...
    # Choose the first isos_req rows from tempdata
    chosen_data = tempdata[:isos_req]

    # Add the chosen rows to the chosen_isolates DF
    chosen_isolates = pd.concat([chosen_isolates, chosen_data])

    # Removes the chosen isolates from tempdata
    tempdata = tempdata[-tempdata["Isolate"].isin(list(chosen_data["Isolate"]))]

    # Removes the chosen isolates from the available_data FD
...
def bugdrug_fill(
    ...
    if not parameters["Bugdrug fill"][0]:
        ...
        for a in abx:
            ...
            # valid data
            # list(row[a].values()) = [SIR, SIGN, VALUE, SCALE]
            # isos_valid contains all isolates where SIR != 0 for this antibiotic
            isos_valid = [
                row["Isolate"]
                for _, row in tempdata.iterrows()
                if list(row[a].values())[0][0] != 0
            ]

            # Selects all rows in tempdata that do have SIR != 0 for this antibiotic
                ...
                for index, row in tempdata_valid.iterrows():
                    ...
                    for k, v in row[a].items():
                        ...
                        if a == "D-test":
                            ...
                            # Remove chosen_isolates from tempdata and available_data
                            tempdata = tempdata[
...
def isolate_selection(available_data, parameters, errors, pats_groups, abx):
    """
    Input:
    - available_data: sorted_dataset
    - parameters: parameters
    - errors: errors
    - pats_groups: pathogen_prio
    - abx: abx
    ...
    for pat_group in pats_groups:
        # Creates a list that contains the subspecies from the parameter_settings.json file
        # [::-2] used to remove "Overall" and "Fastidious" key-value pair
        subspecies = list(parameters["Isolates per species"][pat_group].keys())[:-2]

        for pat in subspecies:
            # print(available_data["Pathogen"] == pat)
            isos_req = parameters["Isolates per species"][pat_group][pat]

            # tempdata stores all rows in the sorted dataset where the pathogen is pat
            # the pathogen we currently are iterating over
...
def iso_sel_setup(available_data, abx, parameters, market_prio):
    ...
    - parameter: argument in main function

    Input:
    - available_data: sorted_dataset
    - abx: abx
    - parameters: parameters
    - market_prio: market_prio
    ...
    [available_data, chosen_isolates, errors] = isolate_selection(
        available_data, parameters, errors, pathogen_prio, abx
    )
...
def main(CIB, parameters, ranges, abx_abbr, market_prio):
    ...
    for d in parameters["Datasets"]:
        try:
            # Reads matrix EU worksheet from CIB Excel file.
            tempdata = pd.read_excel(inputdata, f"matrix {d}")
            data[f"{d}"] = tempdata
        except ValueError:
            print(f"Data region '{d}' not valid, try 'US' or 'EU'")
            continue
        # Add the names of the antibiotics to the list abx
    ...
    for j in range(0, len(data_default)):
        ...
        # Add the isolate and pathogen name of each row to the dictionary as keys
        ...
        # Varför blir isolates_per_species olika för olika j?
        ...
        for k1, v1 in isolates_per_species.items():
            ...
            if break_loop:
                break
        # Add the fastidious state of the pathogen and isolate combination to the dictionary
        res["Fastidious"] = fast
        ...
        for a in abx:
            ...
            # Get data
            # data = matrix EU worksheet
            # j = current row
            # a = current antibiotic
            # fast = fastidious state of pathogen
            final_data = get_data(data, j, a, ranges, abx_abbr, fast, parameters)
            res[a] = final_data
        # Add rank for that isolate
        res = rank_system(res, parameters["Point system"], j)
